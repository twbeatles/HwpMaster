# HWP Master

![Python](https://img.shields.io/badge/Python-3.10+-blue)
![PySide6](https://img.shields.io/badge/PySide6-6.6+-green)
![License](https://img.shields.io/badge/License-MIT-yellow)

**pyhwpx 자동화와 rhwp 내장 편집기를 결합한 HWP 업무 올인원 도구**

HWP Master는 한글(HWP) 문서 작업을 자동화하고, rhwp WebAssembly 기반 내장 편집기로 HWP/HWPX 문서를 열람·편집·저장할 수 있는 윈도우용 데스크톱 애플리케이션입니다.

## ✨ 주요 특징

- **🪶 경량화**: Pandas/NumPy 등 무거운 라이브러리 없이 순수 Python으로 최적화
- **🎨 모던 UI**: PySide6 기반의 세련된 다크모드 대시보드 제공
- **📋 작업 가시성**: 홈 대시보드에서 최근 작업 히스토리, 상태 배지, 즐겨찾기 폴더를 바로 확인
- **✍️ 내장 편집기**: rhwp 0.7.6 WebAssembly 런타임을 QtWebEngine에 self-host하여 HWP/HWPX 문서 편집 워크스페이스 제공
- **🔧 올인원**: 변환, 병합, 분할, 데이터 주입, 서식 교정 등 필수 기능 통합
- **⚡ 고성능**: pyhwpx 기반 자동화와 rhwp 기반 렌더링/편집 경로를 목적에 맞게 분리
- **🛡️ 안전성**: 메타데이터 정리, 배포용 설정, 변경 추적 정리 기능 제공

---

## 📋 시스템 요구사항

| 항목 | 필수 사양 |
|------|----------|
| **OS** | Windows 10/11 (64bit) |
| **소프트웨어** | 자동화 기능은 **한글 2018 이상 설치 필수**, 내장 편집기는 rhwp 런타임으로 동작 |
| **Python** | Python 3.10 이상 |

---

## 🚀 설치 및 실행 방법

### 1. 개발 환경 설정

```bash
# 저장소 클론
git clone https://github.com/twbeatles/HwpMaster.git
cd HwpMaster

# 가상환경 생성 (선택)
python -m venv venv
.\venv\Scripts\activate

# 의존성 설치
pip install -r requirements.txt

# 정적 분석 / 테스트
pyright .
pytest -q
```

### 2. 프로그램 실행

```bash
python main.py
```

### 3. 단독 실행 파일(EXE) 빌드

```bash
# PyInstaller로 EXE 파일 생성
pyinstaller hwp_master.spec
```
빌드 완료 후 결과물은 `dist/` 아래에 생성됩니다.

---

## 🎯 전체 기능 소개

### 1️⃣ 문서 변환 및 관리 (Core)
| 기능 | 설명 |
|------|------|
| **일괄 변환 (Batch Converter)** | HWP 파일을 PDF, TXT, HWPX, JPG 이미지로 일괄 변환합니다. |
| **병합/분할 (Merge & Split)** | 여러 HWP 문서를 하나로 합치거나, 페이지 단위로 쪼갤 수 있습니다. |
| **문서 초기화** | 문서 내부 정보를 정리하고 누름틀을 초기화합니다. |

### 2️⃣ 데이터 자동화 (Data)
| 기능 | 설명 |
|------|------|
| **데이터 주입 (Data Injector)** | 엑셀/CSV 데이터를 HWP 누름틀에 자동 주입합니다. 완전 빈 행만 스킵하며, 출력 파일명 충돌 시 자동 suffix(`_1`, `_2`...)를 붙여 원본 덮어쓰기를 방지합니다. |
| **메타정보 정리** | 문서 작성자, 회사명, 최종 저장 등의 메타데이터를 일괄 삭제하거나 수정합니다. |

### 3️⃣ 서식 및 스타일 (Style)
| 기능 | 설명 |
|------|------|
| **서식 교정 (Style Cop)** | 문서 내 모든 폰트, 줄간격, 자간을 설정된 표준 양식으로 강제 통일합니다. |
| **표 스타일 수정 (Table Doctor)** | 표의 테두리 두께, 셀 여백, 배경색 등을 일괄 변경합니다. |
| **헤더/푸터 관리** | 머리말, 꼬리말, 쪽 번호를 일괄 적용하거나 삭제합니다. |

### 4️⃣ 고급 편집 도구 (Advanced)
| 기능 | 설명 |
|------|------|
| **문서 편집** | rhwp 내장 편집기에서 HWP/HWPX 문서를 열고, 텍스트 삽입, 저장, 다른 이름 저장, 복구본 저장을 수행합니다. HWP 저장은 최초 덮어쓰기 전 백업을 생성하며, HWPX 직접 덮어쓰기는 보호 모드로 제한합니다. |
| **템플릿 스토어** | 8종의 내장 양식(휴가계, 지출결의서 등)을 제공하며, **사용자 지정 파일 등록**도 가능합니다. 필드가 있는 템플릿은 입력값을 받아 바로 새 문서를 생성하고 원본 확장자(HWP/HWPX)를 유지합니다. |
| **매크로 레코더** | 수동 매크로 생성/프리셋 실행뿐 아니라, Action Console 실행 흐름을 **녹화 시작/종료**로 기록해 재실행 가능한 매크로로 저장할 수 있습니다. |
| **정규식 치환** | 주민번호, 전화번호, 이메일 등 민감 정보를 패턴 기반으로 마스킹 처리합니다. |
| **문서 비교 (Doc Diff)** | 두 문서의 내용을 비교하여 차이점을 상세한 HTML 리포트로 생성합니다. |
| **스마트 목차 (Smart TOC)** | form-feed(`\\f`) 기반 페이지 분할로 페이지 번호를 추정하고, 가능 시 HWPX 스타일 힌트를 보조 사용해 목차 정확도를 높입니다. 생성된 텍스트/HTML 목차에는 페이지 번호가 함께 표시되며, 실패 시 패턴 기반으로 자동 폴백합니다. |

### 5️⃣ 유틸리티 (Utils)
| 기능 | 설명 |
|------|------|
| **워터마크** | 텍스트 또는 이미지 워터마크를 투명도 설정과 함께 일괄 삽입합니다. |
| **이미지 추출** | 문서 내 포함된 모든 이미지를 원본 화질로 추출합니다. |
| **링크 검사** | 문서 내 하이퍼링크를 검사하고, 스캔 결과를 확인한 뒤 HTML/XLSX 리포트로 별도 저장할 수 있습니다. |
| **북마크 관리** | 북마크 추출/내보내기, 전체 삭제, **선택 행만 삭제**를 지원합니다. 삭제 기본값은 새 파일 저장(원본 보존)이며, 명시적으로만 덮어쓰기합니다. |

---

## 📝 사용 가이드

### 💡 홈 대시보드 활용하기
1. 앱을 실행하면 홈에서 최근 작업 히스토리와 즐겨찾기 폴더를 바로 확인할 수 있습니다.
2. 즐겨찾기 패널의 `+` 버튼으로 자주 쓰는 폴더를 추가하면 반복 작업 진입이 빨라집니다.
3. 변환, 병합, 템플릿 생성, 액션 콘솔 같은 작업을 마치면 홈으로 돌아왔을 때 최근 작업 패널이 자동으로 갱신됩니다.

### 💡 문서 편집하기
1. 사이드바에서 `✍ 문서 편집` 메뉴를 선택합니다.
2. `문서 열기`로 HWP/HWPX 파일을 선택하면 rhwp 기반 편집 워크스페이스가 열립니다.
3. 편집 후 `저장`, `다른 이름 저장`, `복구본 저장`을 사용할 수 있습니다.
4. HWP 저장은 첫 덮어쓰기 전에 `.hwp_master/editor_backups`에 백업을 생성합니다.
5. HWPX 직접 덮어쓰기는 아직 보호 모드이며, 검증본은 `다른 이름 저장`으로 생성합니다.

### 💡 사용자 템플릿 등록하기
1. 사이드바에서 `📦 템플릿` 메뉴 선택
2. '내장 템플릿' 카드 중 파일이 없는 항목 선택 (예: '연차휴가 신청서')
3. `파일 등록` 버튼을 눌러 내 컴퓨터의 HWP 양식 파일 선택
4. 이제부터 해당 템플릿을 바로 생성하여 사용할 수 있습니다.

### 💡 필드가 있는 템플릿으로 새 문서 만들기
1. 템플릿 카드에서 `사용`을 누르면 저장 경로를 먼저 선택합니다.
2. 템플릿에 누름틀/필드가 정의되어 있으면 입력값 다이얼로그가 열립니다.
3. 입력값을 채우면 새 문서를 생성하며, 출력 파일 확장자는 원본 템플릿과 동일하게 유지됩니다.

### 💡 매크로 프리셋 사용하기
1. 사이드바에서 `🎬 매크로` 메뉴 선택
2. `새 매크로` -> `프리셋에서 가져오기` 클릭
3. '공백 정리', '특수문자 통일' 등 원하는 기능 선택
4. `저장` 후 매크로 목록에서 실행

### 💡 Action Console 실행을 매크로로 녹화하기
1. 사이드바에서 `🎬 매크로` 메뉴에서 `녹화 시작` 클릭
2. `🧰 액션 콘솔`에서 JSON 액션 실행
3. 매크로 페이지로 돌아와 `녹화 종료/저장` 클릭 후 이름 입력
4. 저장된 매크로를 목록에서 즉시 재실행

### 💡 설정에서 환경 진단 실행하기
1. 사이드바에서 `⚙️ 설정` 메뉴 선택
2. 시스템 섹션의 `진단 실행` 클릭
3. `pyhwpx import`, `COM 초기화`, `HWP 기동/종료`, `기본 출력 폴더 쓰기 테스트` 결과 확인
4. 실패 항목이 있으면 상세 로그를 기준으로 런타임/권한 문제를 먼저 점검

---

## 📁 프로젝트 구조

```
HwpMaster/
├── .editorconfig             # UTF-8/LF/들여쓰기 규칙
├── pyrightconfig.json        # Pylance/Pyright 정적 분석 기준
├── LICENSE                   # MIT 라이선스
├── main.py                    # 프로그램 진입점
├── requirements.txt           # 의존성 패키지 목록
├── hwp_master.spec            # PyInstaller 빌드 설정
├── scripts/
│   ├── verify_core_modules.py # 핵심 모듈 임포트 검증
│   └── perf_smoke.py          # 수동 성능 스모크 테스트
├── assets/
│   ├── rhwp_studio/          # 내장 rhwp 편집기 정적 앱 + WASM 런타임
│   └── styles/
│       ├── style.template.qss # 템플릿(토큰) 기반 스타일시트(테마 프리셋 적용)
│       └── style.qss          # 레거시/폴백 스타일시트
├── vendor/
│   └── rhwp/                  # rhwp 0.7.6 npm 산출물과 라이선스 메타데이터
├── src/
│   ├── core/                  # 핵심 비즈니스 로직
│   │   ├── editor/            # rhwp 편집 세션/저장 정책/로컬 asset API 서버
│   │   ├── hwp_handler/       # HWP 제어 파사드 + 내부 도메인 모듈
│   │   ├── action_runner/     # 범용 Run/Execute 액션 실행기 패키지
│   │   ├── capability_mapper.py # pyhwpx 커버리지/능력 매핑
│   │   ├── excel_handler.py   # Excel 처리 (openpyxl)
│   │   ├── template_store/    # 템플릿 저장소 패키지
│   │   ├── macro_recorder/    # 매크로 기록/재생 패키지
│   │   ├── regex_replacer.py  # 정규식 치환
│   │   ├── style_cop.py       # 서식 교정
│   │   ├── table_doctor.py    # 표 스타일 수정
│   │   ├── doc_diff/          # 문서 비교 패키지
│   │   ├── smart_toc.py       # 목차 생성
│   │   ├── watermark_manager.py   # 워터마크 관리
│   │   ├── header_footer_manager.py # 헤더/푸터 관리
│   │   ├── bookmark_manager.py    # 북마크 관리
│   │   ├── hyperlink_checker.py   # 링크 검사
│   │   └── image_extractor.py     # 이미지 추출
│   ├── ui/                    # 사용자 인터페이스
│   │   ├── main_window/       # 메인 윈도우 파사드 + 내부 서브모듈
│   │   ├── pages/             # 기능별 페이지
│   │   │   ├── home_page.py        # 홈 대시보드
│   │   │   ├── editor_page.py      # rhwp 내장 문서 편집기
│   │   │   ├── convert_page.py     # 변환
│   │   │   ├── merge_split_page.py # 병합/분할
│   │   │   ├── data_inject_page.py # 데이터 주입
│   │   │   ├── metadata_page.py    # 메타정보
│   │   │   ├── template_page.py    # 템플릿
│   │   │   ├── macro_page.py       # 매크로
│   │   │   ├── regex_page.py       # 정규식
│   │   │   ├── style_cop_page.py   # 서식 교정
│   │   │   ├── table_doctor_page.py # 표 교정
│   │   │   ├── doc_diff_page.py    # 문서 비교
│   │   │   ├── smart_toc_page.py   # 목차
│   │   │   ├── watermark_page.py   # 워터마크
│   │   │   ├── header_footer_page.py # 헤더/푸터
│   │   │   ├── bookmark_page.py    # 북마크
│   │   │   ├── hyperlink_page.py   # 링크 검사
│   │   │   ├── image_extractor_page.py # 이미지 추출
│   │   │   ├── action_console_page.py # 고급 액션 콘솔
│   │   │   └── settings_page.py    # 설정
│   │   └── widgets/           # 공통 위젯
│   │       ├── file_list.py        # 파일 목록
│   │       ├── feature_card.py     # 기능 카드
│   │       ├── progress_card.py    # 진행률 표시
│   │       ├── sidebar_button.py   # 사이드바 버튼
│   │       ├── page_header.py      # 페이지 헤더
│   │       ├── toast.py            # 알림 메시지
│   │       ├── favorites_panel.py  # 즐겨찾기 패널
│   │       └── history_panel.py    # 작업 히스토리
│   └── utils/                 # 유틸리티
│       ├── atomic_write.py    # 설정/메타데이터 원자적 저장
│       ├── com_init.py        # COM 초기화 컨텍스트
│       ├── filename_sanitizer.py # 안전한 출력 파일명 정리
│       ├── history_manager.py # 작업 히스토리 저장/조회
│       ├── logger.py          # 로깅 시스템
│       ├── output_paths.py    # 출력 경로/충돌 회피 정책
│       ├── qss_renderer.py    # 테마 토큰 기반 QSS 렌더링
│       ├── settings.py        # 설정 관리
│       ├── task_tracking.py   # 최근 파일/작업 이력 기록 헬퍼
│       ├── theme_manager.py   # 테마 관리
│       ├── worker/            # 백그라운드 작업 패키지 (QThread)
│       └── version.py         # 앱 이름/버전 상수
└── tests/                     # 단위 테스트
```

---

## 📚 문서

- 사용자/프로젝트 개요: `README.md`
- 개발 가이드: `CLAUDE.md`
- 구현/운영 컨텍스트: `GEMINI.md`
- 저장소 종합 감사 및 기능 구현 후속 정리: `PROJECT_AUDIT_PYHWPX.md`
- 기능 구현 감사 후속 반영 기록: `FUNCTIONAL_IMPLEMENTATION_AUDIT_2026-04-19.md`
- 정적 분석 기준: `pyrightconfig.json`
- 텍스트 인코딩 규칙: `.editorconfig`

---

## 🔍 개발 품질 게이트

- 정적 분석: `pyright .`
- 회귀 테스트: `pytest -q`
- opt-in 실문서 스모크: `HWPMASTER_REAL_DOC_TESTS=1 pytest -q tests/test_real_hwp_doc_diff_smart_toc.py tests/test_real_hwp_feature_smoke.py`
- 핵심 모듈 임포트 점검: `python scripts/verify_core_modules.py`
- 수동 스모크 점검: `python scripts/perf_smoke.py`

---

## ⚠️ 주의사항

- **한글 자동화 보안 모듈**: 처음 실행 시 보안 모듈 승인 팝업이 뜰 수 있습니다. '승인'을 선택해주세요.
- **백그라운드 실행**: 대량 변환 시 한글(hwp.exe)이 백그라운드에서 실행됩니다. 작업 도중 강제 종료하면 한글 프로세스가 남을 수 있습니다.
- **내장 편집기 저장 정책**: HWP는 최초 덮어쓰기 전에 백업을 생성합니다. HWPX 직접 덮어쓰기는 round-trip 검증 전까지 제한되며, `다른 이름 저장`으로 검증본을 생성합니다.

---

## 📄 라이선스

이 프로젝트는 [MIT License](LICENSE)를 따릅니다. 누구나 자유롭게 수정 및 배포할 수 있습니다.

## 최근 업데이트 (2026-04-27)

- rhwp 기반 내장 편집기 추가
  - `assets/rhwp_studio/`에 self-hosted 편집 UI와 `@rhwp/core 0.7.6` WASM 런타임을 포함
  - `vendor/rhwp/`에 upstream 기준과 MIT 라이선스 메타데이터를 보관
  - `src/core/editor/`에 편집 세션, 저장 서비스, localhost asset/API 서버를 추가
  - `src/ui/pages/editor_page.py`와 사이드바/홈 대시보드에 `문서 편집` 워크스페이스를 연결
- 저장 안정성 보강
  - HWP 현재 저장 시 최초 1회 백업 생성
  - HWPX 직접 덮어쓰기 보호, 복구본 저장 경로 추가
  - 편집 저장 결과를 최근 파일/작업 히스토리에 기록
- 배포/품질 기준 갱신
  - `hwp_master.spec`에 QtWebEngine/QtWebChannel hidden import와 rhwp asset/license 번들링 반영
  - `pyrightconfig.json`에서 `.pytest_tmp` 생성 산출물 제외
  - `tests/test_editor_core.py`로 저장 정책, 백업, 복구본, localhost API token 검증 추가
  - `pytest -q`: `106 passed, 6 skipped`

## 최근 업데이트 (2026-04-19)

- P0/P1 안정화 반영
  - `CapabilityMapper`가 pyhwpx 비가용 snapshot에서도 카테고리 총계를 유지하도록 수정
  - repo-local `pyhwpx` stub과 `pyrightconfig.json` 정리로 정적 분석 기준 복구
  - 템플릿 저장소에 `TemplateStoreError`를 도입해 파일 I/O 실패를 UI 메시지로 일관되게 처리
- 하이퍼링크 검사/저장 정책 재설계
  - 일반 스캔은 메모리 결과만 유지하고, HTML/XLSX 생성은 사용자가 Export를 눌렀을 때만 수행
  - 스캔 오류 시 버튼/프로그레스 상태를 즉시 복구해 UI 잠김을 방지
- 운영성/대시보드 개선
  - 취소 작업도 히스토리에 `cancelled` 상태로 기록하고, 홈 대시보드에 상태 배지 표시
  - 설정 페이지에 `환경 진단` 버튼과 상세 결과 영역 추가
- 패키징/문서 정합성 정리
  - `hwp_master.spec`에 최신 기능 감사 문서를 조건부 번들 목록으로 추가
  - `README.md`, `CLAUDE.md`, `GEMINI.md`, `PROJECT_AUDIT_PYHWPX.md`, `FUNCTIONAL_IMPLEMENTATION_AUDIT_2026-04-19.md` 기준을 2026-04-19 상태로 동기화
- 테스트 확장
  - `tests/test_template_store_errors.py`
  - `tests/test_history_status.py`
  - `tests/test_environment_diagnosis_worker.py`
  - `tests/test_hyperlink_page_state.py`
  - `tests/test_real_hwp_feature_smoke.py`
- 최신 검증 기준
  - `pyright .`: `0 errors, 0 warnings`
  - `pytest -q`: `100 passed, 6 skipped`
  - `HWPMASTER_REAL_DOC_TESTS=1 pytest -q tests/test_real_hwp_doc_diff_smart_toc.py tests/test_real_hwp_feature_smoke.py`: 현재 런타임에서는 `6 skipped`

## 최근 업데이트 (2026-03-25)

- 홈 대시보드와 작업 추적 보강
  - 홈 페이지에 최근 작업 히스토리와 즐겨찾기 폴더 패널을 배치
  - 변환, 병합/분할, 데이터 주입, 메타정보 정리, 템플릿 생성, 매크로 실행, 액션 콘솔 실행 결과를 히스토리에 기록
  - 파일 선택 및 결과 저장 시 최근 파일 목록을 공통 헬퍼로 갱신
- 저장 정합성 강화
  - `settings.json`, `history.json`, 액션 템플릿, 템플릿 메타데이터, 매크로 스크립트 저장을 원자적 쓰기로 통일
  - 템플릿 생성 시 원본 템플릿 확장자와 다른 출력 suffix를 차단하고, 확장자 생략 시 원본 suffix를 자동 부여
- Smart TOC 출력 보강
  - HWPX 스타일 힌트를 실제 페이지 라인에 순차 정렬해 매칭률 통계를 함께 계산
  - HTML 목차에서 제목을 escape 처리하고 페이지 번호를 함께 표기
- 추가 회귀 테스트
  - `tests/test_atomic_storage.py`
  - `tests/test_task_tracking.py`
  - `tests/test_template_page_flow.py`
  - `tests/test_template_store_output_policy.py`
  - `tests/test_smart_toc_paging.py` 보강
  - `tests/test_main_window_lazy_pages.py` 보강
- 당시 검증 기준
  - `pytest -q`: `89 passed, 2 skipped`

## 최근 업데이트 (2026-03-18)

- Pylance/Pyright 정합성 유지
  - `pyright .` 기준 `0 errors, 0 warnings`
  - `ActionRunner` protocol typing, Qt Optional narrowing, page export 정리를 반영
- 같은 import 경로 유지형 패키지 리팩토링
  - `src.core.hwp_handler`, `src.core.action_runner`, `src.core.doc_diff`
  - `src.core.template_store`, `src.core.macro_recorder`
  - `src.utils.worker`, `src.ui.main_window`
- 인코딩 및 텍스트 무결성 보강
  - `src/utils/worker/`, `src/core/hwp_handler/`의 한글 문자열/주석/메시지를 UTF-8 기준으로 정리
  - `tests/test_repository_text_integrity.py`를 추가해 알려진 모지바케 조각과 UTF-8 회귀를 감시
- 문서 및 배포 메타데이터 정리
  - `README.md`, `CLAUDE.md`, `GEMINI.md`, `PROJECT_AUDIT_PYHWPX.md`를 최신 기준으로 갱신
  - `hwp_master.spec`는 실제 존재하는 배포 문서만 조건부로 번들링
- 저장소 운영 설정 보강
  - `.editorconfig`, `.vscode/settings.json`, `.gitignore`를 통해 UTF-8/워크스페이스 인코딩/산출물 제외 정책을 정리
- 최신 검증 기준
  - `pytest -q`: `76 passed, 2 skipped`

## 최근 업데이트 (2026-02-25)

- 메타정보 정리 기능 확장
  - 작성자/주석/변경추적/배포옵션 선택 적용
  - 개인정보 패턴 스캔(주민번호/연락처/이메일 등)
  - 문서 암호 설정(환경 지원 시) + strict 실패 옵션
- 데이터 주입 기능 확장
  - 파일명 생성 필드 지정
  - 파일명 템플릿 지정 (`{부서}_{성명}_{index}` 형식)
- 코어 API 확장
  - `HwpHandler.harden_document`, `scan_personal_info`, `list_fields`, `fill_fields`, `get_meta_tags`, `set_meta_tags`, `mail_merge`

## 최근 업데이트 (2026-02-25, Phase 3/4)

- Phase 3: `execute_action` 기반 빌트인 프리셋 추가
  - `table_professional_style`, `table_dense_grid`
  - `shape_presentation_emphasis`
  - `image_print_enhance`, `image_watermark_light`
  - Action Console에서 프리셋을 JSON으로 불러오기/즉시 실행 가능
- Phase 4: 실문서 기반 정확도 보강 테스트 추가
  - `tests/test_real_hwp_doc_diff_smart_toc.py`
  - 실제 HWP 문서를 생성해 `DocDiff.compare`, `SmartTOC.extract_toc` 검증
  - 기본은 skip, 실행 시 `HWPMASTER_REAL_DOC_TESTS=1` 필요

## 최근 업데이트 (2026-02-27)

- 품질게이트/회귀 안정화
  - QSS 테스트를 테마 토큰 기반 검증으로 전환
- 데이터 주입 안정성 강화
  - CSV 완전 빈 행만 스킵
  - 출력 파일명 충돌 시 자동 suffix 부여
  - 결과 데이터에 `skipped_empty_rows`, `filename_collisions` 포함
- 매크로/북마크 실기능 확장
  - Action Console 실행 명령 녹화 및 매크로 저장
  - 북마크 선택 삭제(`delete_selected`) 구현
- Smart TOC/보안 개선
  - form-feed 기반 페이지 추정 및 `analysis_mode` 제공
  - Doc Diff HTML escape 처리 강화
- 저장 정책 정리(원본 보존 기본)
  - Action Console: `저장 안 함` / `새 파일 저장(기본)` / `원본 덮어쓰기`
  - 북마크 삭제: 기본 새 파일 저장, 덮어쓰기는 명시 선택 시만 허용
- 회귀 테스트(2026-02-27 기준)
  - `pytest -q`: `57 passed, 2 skipped`

## 최근 업데이트 (2026-02-28)

- 기능 구현 감사(섹션 2) 반영 완료
  - 매크로 저장 ID를 `macro_<uuid4hex>`로 전환하여 동시 저장 충돌 제거
  - Header/Footer, Watermark Worker의 partial failure를 `success=False`로 정정
  - remove 모드 출력 경로를 `resolve_output_path()`로 통일해 basename 충돌 회피
  - Excel 행 스킵 정책을 "행 전체가 빈 값일 때만 스킵"으로 통일
  - Hyperlink 리포트 저장 실패를 실패 카운트/최종 실패로 반영
- 회귀 테스트
  - `pytest -q`: `65 passed, 2 skipped`

## 최근 업데이트 (2026-03-03)

- Worker 결과 정합성 개선
  - `HeaderFooterWorker`, `WatermarkWorker`의 성공/실패 판정을 실제 `fail_count` 기반으로 수정
  - 부분 실패 시 실패 파일 요약 메시지 제공
- remove 모드 출력 충돌 회피 강화
  - 헤더/푸터, 워터마크 remove 처리에서 `resolve_output_path(...)`를 사용해 동명 파일 충돌 방지
- 이미지 추출 출력 정책 안정화
  - `ImageExtractWorker`가 `batch_extract` 기반으로 동작하도록 변경
  - 문서별 하위 폴더 출력 정책을 기본 적용해 동명 문서/재실행 충돌 방지
- Doc Diff 임시 파일 정리 보강
  - TEXT fallback 경로를 `try/finally`로 보호해 예외 발생 시에도 임시 파일 삭제 보장
- 매크로 저장 충돌 방지
  - 매크로 ID를 마이크로초 단위(`macro_YYYYmmddHHMMSSffffff`)로 생성
  - ID/스크립트 파일 충돌 시 재생성(fallback) 로직 추가
- 회귀/추가 테스트 보강
  - 신규 테스트 추가:
    - `tests/test_header_footer_worker_result_policy.py`
    - `tests/test_watermark_worker_result_policy.py`
    - `tests/test_remove_mode_output_collision.py`
    - `tests/test_image_extractor_output_collision.py`
    - `tests/test_doc_diff_tempfile_cleanup.py`
    - `tests/test_macro_recorder_id_uniqueness.py`
  - `pytest -q` 기준: `72 passed, 2 skipped`
