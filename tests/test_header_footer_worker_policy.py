...
class _FakeHeaderFooterManagerPartialFail:
    def __enter__(self):
        return self
    ...
    def batch_apply_header_footer(self, files, config, output_dir, progress_callback=None):
        return [
...
class _FakeHeaderFooterManagerRemove:
    ...
    def remove_header_footer(self, source_path: str, output_path: str):
        if output_path:
...
class TestHeaderFooterWorkerPolicy(unittest.TestCase):
    def test_partial_failure_sets_worker_failure(self) -> None:
        files = ["a.hwp", "b.hwp"]
        with (
            patch("src.utils.worker.com_context", return_value=nullcontext()),
            patch("src.core.header_footer_manager.HeaderFooterManager", _FakeHeaderFooterManagerPartialFail),
        ):
    ...
    def test_remove_mode_avoids_output_name_collision(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                patch("src.utils.worker.com_context", return_value=nullcontext()),
                patch("src.core.header_footer_manager.HeaderFooterManager", _FakeHeaderFooterManagerRemove),
            ):
                worker = _CaptureHeaderFooterWorker(
                    mode="remove",
                    files=files,
            ...
            paths = _FakeHeaderFooterManagerRemove.saved_paths
